<script lang="ts">
  ...
  let average = sleepData.reduce((acc, sleep) => acc + sleep.asleep, 0) / sleepData.length;
  ...
  let z = sleepData.map((sleep) => {
    let offsetToBed = new Date(sleep.bedtime).getUTCHours() * 60 + new Date(sleep.bedtime).getUTCMinutes();
  ...
  let avgWeek = ArrWeekday.reduce((acc, sleep) => acc + sleep.asleep, 0) / ArrWeekday.length;
  let avgWeekend = ArrWeekend.reduce((acc, sleep) => acc + sleep.asleep, 0) / ArrWeekend.length;
</script>

<p class="text-lg">
  This is data from <code>{sleepData.length}</code> nights of sleep, tracked with my
...
<p class="text-2xl">
  On weekends, I average <code>{Math.round((avgWeekend * 100) / 3600) / 100} hours</code>
  hours of sleep, on weekdays, I average
  <code>{Math.round((avgWeek * 100) / 3600) / 100} hours</code>.
</p>
<p class="text-lg">
  I certaintly don't remember feeling that well rested, but ok. Also some of these are definitely wrong.
</p>
<div
  ...
  {#each sleepData as sleep}<div
      ...
      <div
        ...
        <span class="block text-sm opacity-60">From {sleep.bedtime} to <br /> {sleep.waketime}</span>
      </div>
    </div>{/each}
</div>
<p>Some of the start times are incorrect due to me taking naps(silly me). Days with no data have been removed.</p>
<div class="flex rounded-sm border border-black/10 p-1">
  {#each z as sleep}
    <div class="flex w-full flex-col bg-stone-100 pt-10 hover:brightness-75">
      <div style={`height: ${sleep.offsetToBed / 10}px`} class="w-full bg-stone-100" />
      <div style={`height: ${sleep.asleep / 60 / 10}px`} class="w-full bg-blue-400" />
    </div>
  {/each}
</div>
<p>And here it shows the time I went to bed and how long I slept. The weekens are really obvious.</p>
<p>
  The sudden jump in the middle is from me moving from Hong Kong to Canada. There's some pretty messed up times here,
  especially during the summer on the left. It's based on UTC time and I don't want to deal with time zones.
</p>
