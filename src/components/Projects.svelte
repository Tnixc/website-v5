<script lang="ts">
  ...
  onMount(() => {
    ...
    const { width: buttonW, height: buttonH, x: buttonX, y: buttonY } = button.getBoundingClientRect();
    const windowLayer = document.getElementById("window-layer")!;
    const pane = windowLayer.appendChild(document.createElement("div"));
    pane.id = "window";
    pane.classList.add("bg-black", "border", "border-black", "fixed", "duration-[600ms]", "max-h-[90vh]");
...
<div id="window-layer" class="pointer-events-none fixed inset-0 z-50 opacity-0 duration-300 ease-out" />
<div class="flex justify-end p-2">
  <button
    class="unblur-in flex gap-4 bg-black px-12 py-6 text-white transition-all cursor-pointer duration-300 hover:scale-105"
    id="projects"
    on:click={() => {
      expand();
    }}>
    <img src={openicon} height="24" width="24" alt="open icon" class="-scale-x-100 hue-rotate-180 invert" />
    Projects</button>
</div>
...
<style>
  #projects {
    box-shadow:
      -20px 125px 51px rgba(0, 0, 0, 0.01),
      -11px 70px 43px rgba(0, 0, 0, 0.05),
      -5px 31px 32px rgba(0, 0, 0, 0.09),
